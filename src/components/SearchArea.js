...
class SearchArea extends React.Component {
    ...
    constructor() {
      ...
      handleSubmit(e) {
        ...
        app.firestore().collection('properties').where('area', '>=', parseInt( this.state.starting ) ).where('area', '<=', parseInt( this.state.ending ) ).get().then((querySnapshot) => {
...
<Header
      ...
      ></Header>
            ...
            <h4 style={{padding: "70px"}}>Search by Area in Meters:</h4>
    <form >
      ...
      <Rect2 style={{paddingLeft: "410px"}}>
        <PlaceholderRow>
          <Placeholder placeholder="Starting Area... 100" type="text" name="searchBox" id="searchBox" value={this.state.starting} onChange={e=> this.setState({starting: e.target.value})}></Placeholder>
          <To>To</To>
          <Placeholder1 placeholder="Ending Area... 2,000" type="text" name="searchBox" id="searchBox" value={this.state.ending} onChange={e=> this.setState({ending: e.target.value})}></Placeholder1>
    ...
    typeof this.state.obj != "undefined" ? 
        ...
        <Link to="/Homepage" style={{marginLeft: "520px"}}  className="btn btn-primary w-50 mt-3 center">
            Go Back Home
          </Link>
